export default {
    "plant1": [
        {
        "plant_name": "plant1_name",
        "water_per_day": 0,
        "water_per_week": 2,
        "environment": "indoors, sunny, room temp",
        "soil_fert": "very very special type of ferilizer, test, test, test",
        "repot": "every 6 months",
        "maintenance_level": "low",
        "fun_fact": "came from blah blah blah",
        "fav": false
        }
    ],

    "plant2": [
        {
            "plant_name": "plant2_name",
            "water_per_day": 1,
            "water_per_week": 7,
            "environment": "indoors, sunny, room temp",
            "soil_fert": "very very special type of ferilizer, test2, test2, test2",
            "repot": "every 1 months",
            "maintenance_level": "high",
            "fun_fact": "came from bleh bleh bleh",
            "fav": false

        }
    ],
    
    "plant3": [
        {
            "plant_name": "plant3_name",
            "water_per_day": 0,
            "water_per_week": 1,
            "environment": "outdoors, in shade, 60-90 degrees",
            "fertilizer": "very very special type of ferilizer, test3, test3, test3",
            "repot": "every 8 months",
            "maintenance_level": "very low",
            "fun_fact": "came from bloh bloh bloh",
            "fav": true

        }
    ],

    "plant4": [
        {
        "plant_name": "plant4_name",
        "water_per_day": 0,
        "water_per_week": 2,
        "environment": "indoors, sunny, room temp",
        "soil_fert": "very very special type of ferilizer, test, test, test",
        "repot": "every 6 months",
        "maintenance_level": "low",
        "fun_fact": "came from blah blah blah",
        "fav": false
        }
    ],

    "plant5": [
        {
            "plant_name": "plant5_name",
            "water_per_day": 1,
            "water_per_week": 7,
            "environment": "indoors, sunny, room temp",
            "soil_fert": "very very special type of ferilizer, test2, test2, test2",
            "repot": "every 1 months",
            "maintenance_level": "high",
            "fun_fact": "came from bleh bleh bleh",
            "fav": false

        }
    ],
    
    "plant6": [
        {
            "plant_name": "plant6_name",
            "water_per_day": 0,
            "water_per_week": 1,
            "environment": "outdoors, in shade, 60-90 degrees",
            "soil_fert": "very very special type of ferilizer, test3, test3, test3",
            "repot": "every 8 months",
            "maintenance_level": "very low",
            "fun_fact": "came from bloh bloh bloh",
            "fav": true

        }
    ],

    "plant7": [
        {
        "plant_name": "plant7_name",
        "water_per_day": 0,
        "water_per_week": 2,
        "environment": "indoors, sunny, room temp",
        "soil_fert": "very very special type of ferilizer, test, test, test",
        "repot": "every 6 months",
        "maintenance_level": "low",
        "fun_fact": "came from blah blah blah",
        "fav": false
        }
    ],

    "plant8": [
        {
            "plant_name": "plant8_name",
            "water_per_day": 1,
            "water_per_week": 7,
            "environment": "indoors, sunny, room temp",
            "soil_fert": "very very special type of ferilizer, test2, test2, test2",
            "repot": "every 1 months",
            "maintenance_level": "high",
            "fun_fact": "came from bleh bleh bleh",
            "fav": false

        }
    ],
    
    "plant9": [
        {
            "plant_name": "plant9_name",
            "water_per_day": 0,
            "water_per_week": 1,
            "environment": "outdoors, in shade, 60-90 degrees",
            "soil_fert": "very very special type of ferilizer, test3, test3, test3",
            "repot": "every 8 months",
            "maintenance_level": "very low",
            "fun_fact": "came from bloh bloh bloh",
            "fav": true

        }
    ]
}